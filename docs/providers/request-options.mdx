...
| `semanticcache.CacheBypassKey`             | `x-bf-cache-bypass`                                  | `bool`                | Skip cache lookup and cache write                                                                                                     |
...
**Context Key:** `semanticcache.CacheBypassKey`
**Header:** `x-bf-cache-bypass`
**Type:** `bool` (header value: `"true"` or `"1"`)
**Required:** No

Skip both cache lookup and cache write. Unlike `CacheNoStoreKey`, this always sends the request to the provider and does not store the response.

<Tabs>
<Tab title="Gateway (cURL)">
```bash
curl --location 'http://localhost:8080/v1/chat/completions' \
--header 'x-bf-cache-bypass: true' \
--header 'Content-Type: application/json' \
--data '{
    "model": "openai/gpt-4o-mini",
    "messages": [{"role": "user", "content": "Hello!"}]
...
```go
ctx := context.Background()
ctx = context.WithValue(ctx, semanticcache.CacheBypassKey, true)
